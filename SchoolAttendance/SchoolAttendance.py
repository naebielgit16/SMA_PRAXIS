...
def register_new_face(frame, face, face_encoding):
    ...
    tk.Label(root, text='Pilih role:').pack()

    # Create an OptionMenu for role selection
    role_var = tk.StringVar(root)
    role_var.set("Student")  # Set default option
    role_options = ["Student", "Mentor", "Visitor"]
    role_menu = tk.OptionMenu(root, role_var, *role_options)
    role_menu.pack()
    ...
    for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
        ...
        if face_key not in detected_faces:
            ...
            if entry:
                ...
                cv2.putText(frame, name_and_role, (left, top-10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 0, 255), 2)
            else:
                register_new_face(frame, face_img, face_encoding)

            cv2.rectangle(frame, (left, top), (right, bottom), (255, 0, 0), 2)

    # Display real-time clock in the video frame
    now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    cv2.putText(frame, now, (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
